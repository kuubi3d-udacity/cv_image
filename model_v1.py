    ...
    def beam_search(self, inputs, k, max_length):
        ...
        for step in range(max_length):
            ...
            for sequence in sequences:
                ...
                embeddings = self.embed(captions).squeeze(0)

                print('image_features.size() = ', image_features.size())
                print('embeddings.size() = ', embeddings.size())


                embeddings = torch.reshape(embeddings, image_features.size())
                #embeddings = torch.reshape(embeddings, (batch_size * k, *image_features.shape[2:]))
                ...
                # Assuming embeddings has shape [batch_size, beam_width, sequence_length, embedding_size]
                #embeddings = embeddings.view(batch_size * k, *embeddings.shape[2:])

                # Remove the '0' dimension using nonzero()
                #nonzeros = embeddings.nonzero()
                #embeddings = embeddings.nonzero()
                #embeddings = embeddings[nonzeros[:, 2]]
                #embeddings = embeddings[:, 2]

                print()
                print('captions', captions.size())
                print('image_features.size() = ', image_features.size())
                #print('nonzeros', nonzeros.size())
                print('embeddings.size() = ', embeddings.size())

                embeddings = torch.cat((image_features.unsqueeze(1).repeat(1, embeddings.shape[0], 1, 1), embeddings.unsqueeze(2)), dim=3)
